...
function card({ recipe, matched, score }) {
  ...
  return `<article class="recipe-card"><img class="recipe-image" src="${escapeHtml(recipe.image)}" alt="${escapeHtml(recipe.name)}" loading="lazy"><div class="card-body"><div class="card-top"><h3>${escapeHtml(recipe.name)}</h3><button class="heart" data-favorite="${recipe.id}" aria-label="${saved ? "Remove" : "Save"} ${escapeHtml(recipe.name)}">${saved ? "♥" : "♡"}</button></div><p class="meta">${escapeHtml(recipe.cuisine)} · ${escapeHtml(recipe.difficulty)} · ★ ${recipe.rating}<br>Prep ${recipe.prepTimeMinutes} min · Cook ${recipe.cookTimeMinutes} min · Serves ${recipe.servings}</p><span class="match">${matched.length} of ${state.ingredients.length} ingredients match (${Math.round(score * 100)}%)</span><p class="matched"><strong>You have:</strong> ${escapeHtml(matched.join(", "))}<br><strong>Still need:</strong> ${escapeHtml(needed.join(", ") || "nothing listed")}</p><button class="details-button" data-details="${recipe.id}" type="button">View recipe</button></div></article>`;
}
...
function openDetails(id) {
  ...
  el.modalContent.innerHTML = `<img class="modal-image" src="${escapeHtml(recipe.image)}" alt="${escapeHtml(recipe.name)}"><div class="modal-body"><h2 id="modal-title">${escapeHtml(recipe.name)}</h2><p class="meta">${escapeHtml(recipe.cuisine)} · ${escapeHtml(recipe.difficulty)} · ★ ${recipe.rating} (${recipe.reviewCount} reviews)</p><div class="detail-grid"><div><strong>Prep</strong>${recipe.prepTimeMinutes} min</div><div><strong>Cook</strong>${recipe.cookTimeMinutes} min</div><div><strong>Serves</strong>${recipe.servings}</div><div><strong>Meal</strong>${escapeHtml((recipe.mealType || []).join(", "))}</div></div><h3>Ingredients</h3><ul>${recipe.ingredients.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ul><h3>Instructions</h3><ol>${recipe.instructions.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ol></div>`;
  el.modal.showModal();
}
